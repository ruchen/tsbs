...
func TestDevopsGroupByTime(t *testing.T) {
	expectedHumanLabel := "MySQL 1 cpu metric(s), random    1 hosts, random 1s by 1m"
	expectedHumanDesc := "MySQL 1 cpu metric(s), random    1 hosts, random 1s by 1m: 1970-01-01T00:05:58Z"
	expectedTable := "cpu"
...
func TestGroupByOrderByLimit(t *testing.T) {
	expectedHumanLabel := "MySQL max cpu over last 5 min-intervals (random end)"
	expectedHumanDesc := "MySQL max cpu over last 5 min-intervals (random end): 1970-01-01T01:16:22Z"
	expectedTable := "cpu"
...
func TestGroupByTimeAndPrimaryTag(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			expectedHumanLabel: "MySQL mean of 1 metrics, all hosts, random 12h0m0s by 1h",
			expectedHumanDesc:  "MySQL mean of 1 metrics, all hosts, random 12h0m0s by 1h: 1970-01-01T00:16:22Z",
			expectedTable: "cpu",
			expectedSQLQuery: `
        WITH cpu_avg AS (
		...
		},
		{
			...
			useTags:            true,
			expectedHumanLabel: "MySQL mean of 1 metrics, all hosts, random 12h0m0s by 1h",
			expectedHumanDesc:  "MySQL mean of 1 metrics, all hosts, random 12h0m0s by 1h: 1970-01-01T00:47:30Z",
			expectedTable: "cpu",
			expectedSQLQuery: `
        WITH cpu_avg AS (
          SELECT time_bucket('3600 seconds', time) as hour, tags_id,
...
func TestMaxAllCPU(t *testing.T) {
	expectedHumanLabel := "MySQL max of all CPU metrics, random    1 hosts, random 8h0m0s by 1h"
	expectedHumanDesc := "MySQL max of all CPU metrics, random    1 hosts, random 8h0m0s by 1h: 1970-01-01T00:16:22Z"
	expectedTable := "cpu"
...
func TestLastPointPerHost(t *testing.T) {
	cases := []struct {
		...
		{
			desc:               "no JSON or tags",
			expectedHumanLabel: "MySQL last row per host",
			expectedHumanDesc:  "MySQL last row per host",
			expectedTable: "cpu",
...
func TestHighCPUForHosts(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			desc:               "zero hosts",
			nHosts:             0,
			expectedHumanLabel: "MySQL CPU over threshold, all hosts",
			expectedHumanDesc:  "MySQL CPU over threshold, all hosts: 1970-01-01T00:16:22Z",
			expectedTable: "cpu",
		...
		},
		{
			desc:               "one host",
			nHosts:             1,
			expectedHumanLabel: "MySQL CPU over threshold, 1 host(s)",
			expectedHumanDesc:  "MySQL CPU over threshold, 1 host(s): 1970-01-01T00:47:30Z",
			expectedTable: "cpu",
		...
		},
		{
			desc:               "five hosts",
			nHosts:             5,
			expectedHumanLabel: "MySQL CPU over threshold, 5 host(s)",
			expectedHumanDesc:  "MySQL CPU over threshold, 5 host(s): 1970-01-01T00:08:59Z",
			expectedTable: "cpu",
...
func verifyQuery(t *testing.T, q query.Query, humanLabel, humanDesc, table, sqlQuery string) {
	tsq, ok := q.(*query.MySQLRequest)

	if !ok {
		t.Fatal("Filled query is not *query.MySQLRequest type")
	}
